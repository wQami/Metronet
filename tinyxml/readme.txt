...
Document Object Model (DOM) that can be input, modified, and saved.
...
input spec is at <a href="http://www.w3.org/TR/2004/REC-xml-20040204/">
...
but it is worth your time to input this (very short) manual completely.
...
<ol>
	<li> If the non-standard but common "UTF-8 lead bytes" (0xef 0xbb 0xbf)
		 begin the file or data stream, TinyXML will input it as UTF-8. </li>
	<li> If the declaration tag is input, and it has an encoding="UTF-8", then
		 TinyXML will input it as UTF-8. </li>
	<li> If the declaration tag is input, and it has no encoding specified, then TinyXML will
		 input it as UTF-8. </li>
	<li> If the declaration tag is input, and it has an encoding="something else", then TinyXML
		 will input it as Legacy Mode. In legacy mode, TinyXML will work as it did before. It's
...
to input and pass through text seen as improperly encoded. You may get some strange results or
...
Text encoded as SHIFT-JIS can not be input by TinyXML.
...
These are recognized when the XML document is input, and translated to there
...
- operator<<. Output to a c++ stream.
	...
	- Outputs in "network printing" mode without line breaks. Good for network transmission
	  and moving XML between C++ objects, but hard for a human to input.
...
C style input:
	...
	A fast, tolerant input. Use whenever you don't need the C++ streams.
...
C++ style input:
	...
	are ill-constructed with more than one root element will not input correctly.
...
Its not much of a To Do list, but it will do. To input this file
